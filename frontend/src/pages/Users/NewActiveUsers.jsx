...
import { useDeleteUser, useGetUserMetrices } from '../../api/ApiCall';
...
export default function NewActiveUsers() {
    ...
    const { data, isLoading, isError } = useGetUserMetrices();
    const { mutate: deleteUser } = useDeleteUser();
    const { mutate: updateStatus } = useUpdateStatus();
    const users = data?.data?.userResponse?.data?.newSignup || [];
    ...
    const fetchAllUsers = async () => {
        try {
            ...
            return response?.data?.data?.userResponse?.data?.newSignup || [];
        } catch (error) {
    ...
    return (
        <Box p={1}>
            <Box display="flex" justifyContent="space-between" mb={2}>
                <Typography variant="h3">New Active User Listing</Typography>
                <Button
                    variant="outlined"
