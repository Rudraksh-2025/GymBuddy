...
export default function Breadcrumbs({
  ...
  const dynamicBreadcrumbMap = [
    {
      match: matchPath('/dashboard/products/:id', location.pathname),
      links: [
        { title: 'Market Place', to: '/dashboard/products' },
        { title: 'Market Place Details', to: location.pathname }
      ]
    },
    {
      match: matchPath('/dashboard/sublease/:id', location.pathname),
      links: [
        { title: 'Sub Lease', to: '/dashboard/sublease' },
        { title: 'Sub Lease Details', to: location.pathname }
      ]
    },
    {
      match: matchPath('/dashboard/users/:id', location.pathname),
      links: [
        { title: 'Users', to: '/dashboard/users' },
        { title: 'User Details', to: location.pathname }
      ]
    }
  ];

  const customLinks =
    dynamicBreadcrumbMap.find((route) => route.match)?.links || null;
  ...
  useEffect(() => {
    ...
    // 🧹 Clear breadcrumbs if nothing matched current route
    if (!found) {
      setMain(undefined);
      setItem(undefined);
    }
    ...
    if (!custom && menu.children) {
      for (let collapse of menu.children) {
        if (collapse.type === 'collapse') {
          const found = getCollapse(collapse);
    ...
    breadcrumbContent = (
      ...
      >
        ...
        >
              ...
              {customLinks.map((link, index) => {
                ...
                return (
                  <Typography
                    ...
                    color={!link.to ? 'text.primary' : 'text.secondary'}
                  >
                    {link.icon && <LinkIcon style={iconSX} />}
                    {link.title}
                  </Typography>
                );
              })}
          ...
          {title && titleBottom && (
            <Grid sx={{ mt: card === false ? 0.25 : 1 }}>
              {/* <Typography variant="h2">{customLinks[customLinks.length - 1].title}</Typography> */}
            </Grid>
          )}
        </Grid>
        {card === false && divider !== false && <Divider sx={{ mt: 2 }} />}
  ...
  const hiddenRoutes = ['/dashboard/view-profile'];
